	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			rows := mockDB.NewRows([]string{"id", "name", "description", "preview_path",
				"preview_video_path", "release_year", "rating", "age_restriction", "seasons_count"}).
				AddRow(test.videos[0].ID, test.videos[0].Name, test.videos[0].Description,
					...
					test.videos[0].Rating, test.videos[0].AgeRestriction, test.videos[0].SeasonsCount).
				AddRow(test.videos[1].ID, test.videos[1].Name, test.videos[1].Description,
					...
					test.videos[1].Rating, test.videos[1].AgeRestriction, test.videos[1].SeasonsCount)
